...
fun typecheck (PROG(_,decleration_seq,command_seq)) =
    (makeSymbolTable(decleration_seq);checkCommandSeq(command_seq))

and checkCommandSeq([]) = true
    |checkCommandSeq(commandSequence: CMD list) = 
        (case List.find (fn x => checkCommand(x) = false) commandSequence of 
            SOME _ => raise invalidCommand
            |NONE => true)
        handle invalidCommand => (print("Command is invalid \n");false)
